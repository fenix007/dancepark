$(document).ready(function(){
    ...
    function refreshRegisterForm (data) {
        ...
        function registerAuthorized(data)
        {

            var approvedRegistration = $('.register-approved');
//        $(approvedRegistration).append('<div class="d-none">Ваша заявка принята - ' +
//            'на указанный email отправлено письмо для подтверждения регистрации.</div>');
            $(approvedRegistration).fadeIn('slow');
            window.setTimeout(function(){$(approvedRegistration).hide('slow')}, 2000);
        }
    };
